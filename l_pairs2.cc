...
void l_pairs2() {
    // Open ROOT file
    std::unique_ptr<TFile> f(TFile::Open("output_6.root", "READ"));
    if (!f || f->IsZombie()) {
    ...
    std::unique_ptr<TH2F> h2_lambda_lambda(new TH2F("h2_lambda_lambda", "Lambda-Lambda Invariant Mass", 50, 0, 3, 50, 0, 3));
    std::unique_ptr<TH2F> h2_lambda_lambda_bar(new TH2F("h2_lambda_lambda_bar", "Lambda-Lambda-bar Invariant Mass", 50, 0, 3, 50, 0, 3));
    std::unique_ptr<TH2F> h2_lambda_bar_lambda_bar(new TH2F("h2_lambda_bar_lambda_bar", "Lambda-bar-Lambda-bar Invariant Mass", 50, 0, 3, 50, 0, 3));
        ...
        if (eventId != prevEventID && prevEventID != -1) {
            for (size_t j = 0; j < lambdas.size(); ++j) {
                for (size_t k = j + 1; k < lambdas.size(); ++k) {
                    ...
                    double mass = invariantMass(lambdas[j], lambdas[k]);
                    h2_lambda_lambda->Fill(lambdas[j].Pt(), lambdas[k].Pt(), mass);
                }
                ...
                for (size_t k = 0; k < lambda_bars.size(); ++k) {
                    ...
                    double mass = invariantMass(lambdas[j], lambda_bars[k]);
                    h2_lambda_lambda_bar->Fill(lambdas[j].Pt(), lambda_bars[k].Pt(), mass);
                }
            }
            ...
            for (size_t j = 0; j < lambda_bars.size(); ++j) {
                for (size_t k = j + 1; k < lambda_bars.size(); ++k) {
                    ...
                    double mass = invariantMass(lambda_bars[j], lambda_bars[k]);
                    h2_lambda_bar_lambda_bar->Fill(lambda_bars[j].Pt(), lambda_bars[k].Pt(), mass);
                }
            }
