...
void lpair_check2() {
    ...
    for (int i = 0; i < nEntries; ++i){
        ...
        // Pair charge check
        if (pair_charge == 0){
            lambda_us.push_back(p1);
            lambda_bar_us.push_back(p2);
        } else{
            lambda_ls.push_back(p1);
            lambda_bar_ls.push_back(p2);
        }

        // Sort into lambdas and lambda bars
        if (p1_ch > 0){
    ...
    for (size_t idx = 0; idx < std::min({lambda_ls.size(), lambda_us.size(), lambda_bar_ls.size(), lambda_bar_us.size()}); ++idx) {
        h1->Fill(lambda_us[idx], lambda_us[idx]);
        h2->Fill(lambda_us[idx], lambda_ls[idx]);
        h3->Fill(lambda_ls[idx], lambda_ls[idx]);
        h4->Fill(lambda_ls[idx], lambda_us[idx]);
        h5->Fill(lambda_us[idx], lambda_bar_us[idx]);
        h6->Fill(lambda_us[idx], lambda_bar_ls[idx]);
        h7->Fill(lambda_ls[idx], lambda_bar_ls[idx]);
        h8->Fill(lambda_ls[idx], lambda_bar_us[idx]);
        h9->Fill(lambda_bar_us[idx], lambda_bar_us[idx]);
        h10->Fill(lambda_bar_us[idx], lambda_bar_ls[idx]);
        h11->Fill(lambda_bar_ls[idx], lambda_bar_ls[idx]);
        h12->Fill(lambda_bar_ls[idx], lambda_bar_us[idx]);
    }
