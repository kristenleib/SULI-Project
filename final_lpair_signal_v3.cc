...
void final_lpair_signal_v3() {
    ...
    // M_inv histograms
    TH2F *mixed_event_SE_lambda_us_S_lambda_us = new TH2F("mixed_event_SE_lambda_us_S_lambda_us", "Unlike Sign Lambda - Unlike Sign Lambda (Includes Background)", 500, 1.0, 1.4, 500, 1.0, 1.4);
    TH2F *mixed_event_SE_lambda_us_S_lambda_ls = new TH2F("mixed_event_SE_lambda_us_S_lambda_ls", "Unlike Sign Lambda - Like Sign Lambda (Background)", 500, 1.0, 1.4, 500, 1.0, 1.4);
    TH2F *mixed_event_SE_lambda_us_S_lambda_bar_us = new TH2F("mixed_event_SE_lambda_us_S_lambda_bar_us", "Unlike Sign Lambda - Unlike Sign Lambda Bar (Includes Background)", 500, 1.0, 1.4, 500, 1.0, 1.4);
    TH2F *mixed_event_SE_lambda_us_S_lambda_bar_ls = new TH2F("mixed_event_SE_lambda_us_S_lambda_bar_ls", "Unlike Sign Lambda - Like Sign Lambda Bar (Background)", 500, 1.0, 1.4, 500, 1.0, 1.4);
    TH2F *mixed_event_SE_lambda_bar_us_S_lambda_bar_us = new TH2F("mixed_event_SE_lambda_bar_us_S_lambda_bar_us", "Unlike Sign Lambda Bar - Unlike Sign Lambda Bar (Includes Background)", 500, 1.0, 1.4, 500, 1.0, 1.4);
    TH2F *mixed_event_SE_lambda_bar_us_S_lambda_bar_ls = new TH2F("mixed_event_SE_lambda_bar_us_S_lambda_bar_ls", "Unlike Sign Lambda Bar - Like Sign Lambda Bar (Background)", 500, 1.0, 1.4, 500, 1.0, 1.4);
    ...
    // *********************** Adjust this *********************** 
    TF2 *gaus2D_ME_LL = new TF2("gaus2D_ME_LL", "[0]*TMath::Gaus(x,[1],[2])*TMath::Gaus(y,[3],[4])", minX_ME, maxX_ME, minY_ME, maxY_ME);
    mixed_event_SE_lambda_us_S_lambda_us->Fit(gaus2D_ME_LL, "R");

    // Retrieve the mean and sigma values from the Gaussian fit
    double meanX_ME = gaus2D_ME_LL->GetParameter(1);
    double sigmaX_ME = gaus2D_ME_LL->GetParameter(2);
    double meanY_ME = gaus2D_ME_LL->GetParameter(4);
    double sigmaY_ME = gaus2D_ME_LL->GetParameter(5);

    // Calculate the 3-sigma range
    double minX_range_ME = meanX_ME - 3.0 * sigmaX_ME;
    double maxX_range_ME = meanX_ME + 3.0 * sigmaX_ME;
    double minY_range_ME = meanY_ME - 3.0 * sigmaY_ME;
    double maxY_range_ME = meanY_ME + 3.0 * sigmaY_ME;

    // Loop over Lambda-Lambda pairs and apply the checks
    for (size_t j = 0; j < same_event_lambda_us_1.size(); ++j) {
        for (size_t k = 0; k < single_lambda_us.size(); ++k) {
            if (proton_id_lambda_us[j] != proton_id_lambda_us[k] || pion_id_lambda_us[j] != pion_id_lambda_us[k]) {
                ...
                double invariantMass_same_event_lambda_us_1 = same_event_lambda_us_1[j].M();
                double invariantMass_single_lambda_us_1 = single_lambda_us[k].M();

                // Check if invariant mass values are within the 3-sigma range
                if (invariantMass_same_event_lambda_us_1 >= minX_range_ME && invariantMass_same_event_lambda_us_1 <= maxX_range_ME &&
                    invariantMass_single_lambda_us_1 >= minY_range_ME && invariantMass_single_lambda_us_1 <= maxY_range_ME) {

                    double delta_eta = std::abs(same_event_lambda_us_1[j].Eta() - single_lambda_us[k].Eta());
                    double delta_phi = std::abs(same_event_lambda_us_1[j].Phi() - single_lambda_us[k].Phi());
                    ...
                    mixed_event_SE_lambda_us_S_lambda_us->Fill(invariantMass_same_event_lambda_us_1, invariantMass_single_lambda_us_1);

                    // Boost protons to Lambda rest frame and calculate theta_star
                    same_event_proton_us_1[j].Boost(-same_event_lambda_us_1[j].BoostVector());
                    single_proton_us[k].Boost(-single_lambda_us[k].BoostVector());
                    double ME_theta_star = same_event_proton_us_1[j].Angle(single_proton_us[k].Vect());
