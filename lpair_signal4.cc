...
void lpair_signal4(){
    ...
    // vector<double> cos_theta_star_values;
    // vector<double> cos_theta_star2_values;
    // vector<double> cos_theta_star3_values;
    // vector<double> cos_theta_star4_values;
    // vector<double> cos_theta_star5_values;
    // vector<double> cos_theta_star6_values;
    ...
    // TH1D *h_cos_theta_starLL = new TH1D("h_cos_theta_starLL", "Angular Correlation of Lambda - Lambda Signal (US-US)", 10, -1, 1);
    // TH1D *h_cos_theta_starLLBar = new TH1D("h_cos_theta_starLLBar", "Angular Correlation of Lambda - Lambda Bar Signal (US-US)", 10, -1, 1);
    // TH1D *h_cos_theta_starLBarLBar = new TH1D("h_cos_theta_starLBarLBar", "Angular Correlation of Lambda Bar - Lambda Bar Signal (US-US)", 10, -1, 1);
    ...
    // Lambda Bar - Lambda Bar Candidates
    c1->cd(3);
    ...
    //Lambda - Lambda Pair Polarization Calculations
    // for loop for lambda_us - lamdba_us
    ...
    TF1 *fit_cos_theta_starLL = new TF1("fit_cos_theta_starLL", "[0] * (1 + [1] * cos(x))", -1, 1);
    fit_cos_theta_starLL->SetParameters(1, 1); // Initial guess for A and B
    ...
    // Lambda - Lambda Bar Pair Polarization Calculations
    // for loop for lambda_us - lambda_bar_us
    ...
    // Define the fitting function
    TF1 *fit_cos_theta_starLLBar = new TF1("fit_cos_theta_starLLBar", "[0] * (1 + [1] * cos(x))", -1, 1);
    ...
    // Lambda Bar - Lambda Bar Pair Polarization Calculations
    // for loop for lambda_bar_us - lambda_bar_us
    ...
    // Define the fitting function
    TF1 *fit_cos_theta_starLBarLBar = new TF1("fit_cos_theta_starLBarLBar", "[0] * (1 + [1] * cos(x))", -1, 1);
    ...
    std::unique_ptr<TCanvas> c4(new TCanvas("c4", "Angular Distributions", 1800, 900));
    c4->Divide(3, 1);
    ...
    // Draw histograms with error bars on first pad
    c4->cd(1);
    h_cos_theta_star->SetMinimum(0);
    h_cos_theta_star->Draw("PE"); // Draw with error bars
    h_cos_theta_star->SetTitle("Lambda - Lambda Angular Correlation");
    
    h_cos_theta_star2->SetLineColor(kRed);
    h_cos_theta_star2->Draw("same PE"); // Draw with error bars

    // Draw histograms with error bars on second pad
    c4->cd(2);
    ...
    // Draw histograms with error bars on third pad
    c4->cd(3);
    ...
    h_cos_theta_starLL->SetTitle("Lambda - Lambda Aingular Correlation (signal)");
    ...
    h_cos_theta_starLLBar->SetTitle("Lambda - Lambda Bar Aingular Correlation (signal)");
    ...
    h_cos_theta_starLBarLBar->SetTitle("Lambda Bar - Lambda Bar Aingular Correlation (signal)");
    fit_cos_theta_starLLBar->Draw("same");
